...
- 🎛️ **多通道 Coding Plan 自由轮播**：
  ...
  - **GPT（ChatGPT Plus / Pro）**：应用内 ChatGPT OAuth 登录，按账号套餐显示 `Plus`、`Pro 5x` 或 `Pro 20x` 标识及实际 `5H余额` / `本周余额`，不依赖官方 Codex 客户端。Pro $100/月为 `Pro 5x`（接口值 `prolite`），Pro $200/月为 `Pro 20x`（接口值 `pro`）。
...
- ⏱️ **严格 60 秒同步并发调度**：多通道严格在同一时刻并发发起请求，网络耗时完全不影响 60 秒的固定周期，数据高度同步。
