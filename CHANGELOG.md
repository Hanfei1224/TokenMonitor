...
### Added
- 每个供应商支持添加多个账号或 API 配置，并可在设置中命名、重命名和删除。
- 主界面支持使用上下箭头切换当前供应商内的账号或 API。
- Gemini 和 GPT 套餐标志按套餐类型使用不同颜色显示。

### Changed
- 每轮刷新会获取所有已配置账号的额度；每个通道最多同时请求 5 个账号，账号完成请求后立即推送对应数据，避免等待整批账号完成。
- Gemini 套餐判定参考 Antigravity-Manager：按 `paidTier`、`currentTier` 和受限账号的 `allowedTiers` 进行归类；已返回但无法细分的套餐归类为 `Free`，没有套餐字段时不显示。
- 四个通道的左右切换箭头统一尺寸和垂直布局。
- 旧版单账号配置会自动迁移为账号列表，保留原有授权和 API Key。
- 本地 Token 统计改为按文件保存扫描状态：未变化日志直接跳过，追加内容只读取新增部分，改写或截断的文件自动重新核对。
- SQLite 统计保留增量读取，并自动校准最近两天；开发版和安装版继续使用独立缓存，避免互相污染。

### Fixed
- 修复日志读取失败或延迟写入后扫描时间仍推进，导致安装版与开发版 Token 统计结果长期不一致的问题。
- 统计缓存升级时自动备份旧缓存并重建，避免历史漏算数据继续沿用。
## [1.3.4] - 2026-09-02
